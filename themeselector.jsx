...
const themeMessages = {
    // ATHLETES (Jr High, High School, College)
    athletes: {
        label: "Athletes",
    ...
    // RECREATIONAL (Weekend warriors, casual exercisers)
    recreational: {
        label: "Recreational",
    ...
    // RUNNERS (Distance, sprinters, casual runners)
    runners: {
        label: "Runners",
    ...
    // GOLFERS (Recreational to competitive)
    golfers: {
        label: "Golfers",
    ...
    // FITNESS (General fitness enthusiasts)
    fitness: {
        label: "Fitness",
...
// Main component
const ThemeSelector = ({ onSelect, currentTheme, currentCategory, weekNumber }) => {
    const [isOpen, setIsOpen] = useState(false);
    const [selectedCategory, setSelectedCategory] = useState(currentCategory || 'athletes');
    const [selectedTheme, setSelectedTheme] = useState(currentTheme || 'testing');
                    ...
                    <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                        ...
                        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white px-6 py-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h2 className="text-xl font-bold">Weekly Theme Selector</h2>
                                    <p className="text-blue-100 text-sm">Choose your client category and training focus</p>
                                </div>
                                <button
                                    ...
                                    {categories.map((cat) => (
                                        <button
                                            ...
                                            onClick={() => {
                                                ...
                                                // Reset theme if not available in new category
                                                if (!themeMessages[cat].themes[selectedTheme]) {
                                                    setSelectedTheme(Object.keys(themeMessages[cat].themes)[0]);
                                                }
                                                setSelectedMessage(null);
                                            }}
                                            className={`px-4 py-3 rounded-lg border-2 transition-all text-center ${selectedCategory === cat
                                                    ? 'border-blue-600 bg-blue-50 text-blue-700'
                                                    : 'border-gray-200 hover:border-gray-300 text-gray-700'
                                                }`}
                                        >
                                            <div className="font-semibold text-sm">{themeMessages[cat].label}</div>
                                            <div className="text-xs text-gray-500 mt-1">{themeMessages[cat].sublabel}</div>
                                        </button>
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {getAvailableThemes().map((theme) => (
                                        <button
                                            ...
                                            className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${selectedTheme === theme
                                ...
                                <div className="space-y-3">
                                    {getMessages().map((message, index) => (
                                        <div
                                            ...
                                            className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${selectedMessage === message
                                                    ? 'border-blue-600 bg-blue-50'
                                                    : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                                                }`}
                                        >
                                            <div className="flex items-start justify-between gap-4">
                                                <p className="text-gray-800 flex-1">{formatMessage(message)}</p>
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                        ...
                        <div className="border-t bg-gray-50 px-6 py-4 flex items-center justify-between">
                            <div className="text-sm text-gray-600">
                                {selectedMessage ? (
                                    <span className="text-green-600 font-medium">✓ Message selected</span>
                                ) : (
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    className={`px-6 py-2 rounded-lg font-medium transition-colors ${selectedMessage
